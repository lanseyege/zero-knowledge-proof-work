...
pub fn get_config(file_name : &str) -> (usize, u32, u32, u32, u32, u32) {
    let mut file = File::open(file_name).unwrap();
    let mut contents = String::new();
    ...
    (bit_length, p_length, witness, prime_reps, is_random, rand_seed)
}
...
pub fn read_variables(file_name : &str, _p : &mut Integer, _q : &mut Integer, _g : &mut Integer, _h : &mut Integer, _u : &mut Integer, _vec_g : &mut Vec<Integer>, _vec_h : &mut Vec<Integer>) {
...
pub fn get_t1(al : &Vec<Integer>, ar : &Vec<Integer>, SL : &Vec<Integer>, SR : &Vec<Integer>, y : &Integer, z : &Integer, lens : usize, modulo : &Integer, res : &mut Integer) {
    ...
    for i in 0..lens {
        temp = temp + ((al[i].clone() - z.clone()) * (_y.clone() * SR[i].clone()) + SL[i].clone() * (_y.clone() * (ar[i].clone() + z.clone()) + z.clone() * z.clone() * _tw.clone()));
