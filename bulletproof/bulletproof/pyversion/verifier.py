...
class Verifier():
    ...
    def verifier_range_proof(self, _tau_x, _mu, _hat_t, _V, _A, _S, _T1, _T2, is_output):
        ...
        delta = get_delta(_random_z, _random_y, lens, self._p)
        check_tr = (pow(_V, _random_z * _random_z, self._q) * pow(self._g, delta, self._q) %self._q) * (pow(_T1, _random_x, self._q) * pow(_T2, _random_x * _random_x, self._q) %self._q) % self._q
        ...
        vec_temp = get_vec_temp(_random_y, _random_z, lens, self._p)
        gz = 1
        for i in range(lens):
            a = pow(self._vec_g[i], _random_z, self._q)
            b = pow(a, -1, self._q)
            gz *= b
        gz %= self._q
        hp = 1
        for i in range(lens):
            ...
            hp *= a
        hp %= self._q
        PL = _A * pow(_S, _random_x, self._q) * gz * hp % self._q
        a = pow(self._h, _mu, self._q)
        self._P = pow(a, -1, self._q) * PL % self._q
        self._vec_h = _vec_hi[:]
        print("verifier _vec_hi")
        print(_vec_hi)
        if is_output:
